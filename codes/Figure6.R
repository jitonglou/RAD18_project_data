#This pipeline is applicative for data sets downloaded from TCGA portal (https://portal.gdc.cancer.gov/).
#TCGA-LUAD would be the sample data set in this R script.
###Load the packages###
library(tidyverse)
...
library(maftools)
###Data Preparation###
#The following data were all downloaded on TCGA portal.
...
##Comparison of RAD18 mRNA expression between tumor tissues and normal tissues (As in Fig 6A)
...
##Comparison of RAD18 expression among RAD18 CNV status (As in Fig 6B)
figb <- radcnv
...
##Histogram of smoking dose. (As in Fig 6C)
ggplot(smokerlist)+
  stat_count(aes(cigarettes_per_day),geom = 'bar',width = 0.3)

##Comparison of total mutation counts grouped by smoking history and RAD18 expression. (As in Fig 6D)
...
##Comparison of mutation counts grouped by smoking history and RAD18 expression. (As in Fig 6E)
...
##Scatter plot of total mutation counts and RAD18 mRNA expression. (As in Fig 6F)
ggplot(dc,aes(study,sum))+geom_jitter()+facet_wrap(~cigar) +
  labs(x="RAD18 mRNA Expression",y="log2(SNV Counts+1)")+
  geom_smooth()+stat_cor()

##Scatter plot of mutation counts and RAD18 mRNA expression. (As in Fig 6G)
ggplot(cc,aes(study,value))+geom_jitter()+facet_wrap(~variable+cigar) +
  labs(x="RAD18 mRNA Expression",y="log2(SNV Counts+1)")+
  geom_smooth()+stat_cor()
  
##Mutation Spectrum Grouping by RAD18 Expression Level and Smoking History. (As in Fig 6H)
plotMutationSpectrum(RADEXP_motifs,group = "study")
